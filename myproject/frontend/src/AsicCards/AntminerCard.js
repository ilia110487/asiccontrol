...
const AntminerCard = ({ device, onEditDevice, onDeleteDevice }) => {
    ...
    if (!minerData) return <div className="antminer-card">Загрузка...</div>;

    const chainTemps = minerData.chains.map((chain) =>
        Math.max(...chain.temp_chip)
    );
...
<div className="chains-section">
    ...
    <div className="chain-bars">
        {chainTemps.map((temp, index) => (
            <div key={index} className="chain-item">
                <div
                    className="chain-bar"
                    style={{
                        width: `${temp}%`,
                        backgroundColor: getBarColor(temp, "temp"),
                    }}
                ></div>
